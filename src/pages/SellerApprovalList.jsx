...
const SellerApprovalList = () => {
    ...
    // 🔥 Fetch sellers with better error handling
    const fetchSellers = async () => {
        ...
        try {
            ...
            console.log("✅ Sellers loaded:", data.length, "items");
            ...
            setFetchError(error.message || "Failed to load sellers");
            toast.error("Failed to load sellers. Please refresh.");
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        console.log("🔍 Filtering sellers with:", { searchTerm, activeTab, totalSellers: sellers.length });
        ...
        let filtered = sellers.filter((seller) => {
            ...
            if (activeTab === "reapplied") return seller.status === 4;
            return true;
        });
    ...
    const confirmStatusChange = async () => {
        ...
        try {
            console.log(`🔄 Updating seller ${sellerId} to status ${newStatus}`);
    ...
    const getStatusInfo = (status) => {
        const statusMap = {
            ...
            4: { label: "Re-applied", color: "bg-indigo-100 text-indigo-700", icon: IoRepeatOutline },
        };
    ...
    const getCounts = () => {
        ...
        const reapplied = sellers.filter(s => s.status === 4).length;
        return { total, pending, approved, rejected, reapplied };
    };
        ...
        return (
            <>
                ...
                <div className={clsx('relative', 'min-h-screen', 'bg-white', 'overflow-x-hidden', 'p-6', 'mt-5')}>
                    <div className={clsx('flex', 'flex-col', 'justify-center', 'items-center', 'h-64')}>
                        <div className={clsx('animate-spin', 'rounded-full', 'h-12', 'w-12', 'border-b-2', 'border-emerald-600')}></div>
                        <p className={clsx('mt-4', 'text-gray-500', 'font-medium')}>Loading sellers...</p>
                    </div>
                </div>
            </>
        );
    ...
    return (
        <>
            ...
            <div className={clsx('relative', 'min-h-screen', 'bg-white', 'overflow-x-hidden', 'p-6', 'mt-5')}>
                ...
                <div className={clsx('relative', 'z-10', 'flex', 'flex-col', 'gap-12', 'items-center')}>
                    <div className="w-full">
                        {/* Header */}
                        <div className={clsx('flex', 'flex-col', 'md:flex-row', 'justify-between', 'items-center', 'mb-6', 'gap-6', 'px-6')}>
                            <h2 className={clsx('text-3xl', 'font-bold', 'text-green-600', 'flex', 'items-center', 'gap-3')}>
                                Seller Management
                            </h2>
                            <div className={clsx('flex', 'items-center', 'gap-4', 'w-full', 'md:w-auto')}>
                                <div className={clsx('relative', 'w-full', 'md:w-96', 'group', 'flex-1')}>
                                    <div className={clsx('absolute', 'inset-y-0', 'left-0', 'pl-4', 'flex', 'items-center', 'pointer-events-none')}>
                                        <TbUser className={clsx('h-6', 'w-6', 'text-emerald-500', 'font-bold')} />
                                    </div>
                                    <input
                                        type="text"
                                        value={searchTerm}
                                ...
                                <button
                                    ...
                                    className={clsx('p-3', 'bg-emerald-500', 'text-white', 'rounded-2xl', 'hover:bg-emerald-600', 'transition-all', 'shadow-md', 'hover:shadow-lg', 'flex', 'items-center', 'gap-2')}
                                    title="Refresh sellers"
                                >
                                    <IoRefreshOutline size={20} />
                                    <span className={clsx('hidden', 'sm:inline', 'text-sm', 'font-medium')}>Refresh</span>
                                </button>
                            </div>
                        </div>

                        {/* Tabs */}
                        <div className={clsx('flex', 'flex-wrap', 'gap-2', 'px-6', 'mb-6')}>
                            <button
                            ...
                            <button
                                onClick={() => setActiveTab("reapplied")}
                                className={clsx(
                                    "px-5 py-2.5 rounded-xl font-medium text-sm transition-all duration-200 flex items-center gap-2",
                                    activeTab === "reapplied"
                            ...
                            >
                                ...
                                <span className={clsx(
                                    "ml-1 px-2 py-0.5 rounded-full text-xs",
                                    activeTab === "reapplied" ? "bg-white/20 text-white" : "bg-gray-200 text-gray-600"
                                )}>
                                    {counts.reapplied}
                                </span>
                            </button>
                        ...
                        {/* Table */}
                        <div className={clsx('backdrop-blur-xl', 'p-8', 'overflow-hidden', 'rounded-[40px]')}>
                            {fetchError && (
                                <div className={clsx('mb-4', 'p-4', 'bg-rose-50', 'border', 'border-rose-200', 'rounded-2xl', 'text-rose-700', 'text-center')}>
                                    <p className="font-medium">⚠️ {fetchError}</p>
                                    <button 
                                        onClick={fetchSellers}
                                        className={clsx('mt-2', 'px-4', 'py-2', 'bg-rose-600', 'text-white', 'rounded-xl', 'hover:bg-rose-700', 'transition')}
                                    >
                                        Try Again
                                    </button>
                                </div>
                            ...
                            {filteredSellers.length === 0 && !loading && !fetchError ? (
                                <div className={clsx('text-center', 'py-12')}>
                                    <div className={clsx('text-6xl', 'mb-4')}>📭</div>
                                    <h3 className={clsx('text-2xl', 'font-bold', 'text-gray-700')}>No Sellers Found</h3>
                                    <p className={clsx('text-gray-500', 'mt-2')}>
                                        {searchTerm ? "Try adjusting your search" : "No sellers in this category"}
                                    </p>
                                    {sellers.length === 0 && (
                                        <p className={clsx('text-gray-400', 'mt-1', 'text-sm')}>
                                            Total sellers in system: 0. Please check your backend data.
                                        </p>
                                    )}
                                    <button 
                                        onClick={fetchSellers}
                                        className={clsx('mt-4', 'px-6', 'py-2', 'bg-emerald-500', 'text-white', 'rounded-xl', 'hover:bg-emerald-600', 'transition')}
                                    >
                                        <IoRefreshOutline className={clsx('inline', 'mr-2')} />
                                        Refresh
                                    </button>
                                </div>
                            ) : (
                                <>
                                    <div className="overflow-x-auto">
                                        <table className={clsx('min-w-full', 'divide-y', 'divide-gray-200')}>
                                            <thead className={clsx('bg-gradient-to-r', 'from-emerald-50', 'via-teal-50', 'to-cyan-50', 'border-b-2', 'border-emerald-200', 'rounded-t-xl')}>
                                                <tr>
                                                    <th className={clsx('px-6', 'py-4', 'text-left', 'rounded-tl-xl')}>
                                                        <div className={clsx('flex', 'items-center', 'gap-2.5', 'text-emerald-800', 'font-bold', 'text-xs', 'uppercase', 'tracking-wider')}>
                                                            <IoBusinessOutline size={16} className="text-emerald-600" />
                                                            Business
                                                        </div>
                                                    </th>
                                                    <th className={clsx('px-6', 'py-4', 'text-left')}>
                                                        <div className={clsx('flex', 'items-center', 'gap-2.5', 'text-emerald-800', 'font-bold', 'text-xs', 'uppercase', 'tracking-wider')}>
                                                            <IoPersonOutline size={16} className="text-emerald-600" />
                                                            Owner
                                                        </div>
                                                    </th>
                                                    <th className={clsx('px-6', 'py-4', 'text-left', 'hidden', 'md:table-cell')}>
                                                        <div className={clsx('flex', 'items-center', 'gap-2.5', 'text-emerald-800', 'font-bold', 'text-xs', 'uppercase', 'tracking-wider')}>
                                                    ...
                                                    <th className={clsx('px-6', 'py-4', 'text-center', 'rounded-tr-xl')}>
                                                        <div className={clsx('flex', 'items-center', 'justify-center', 'gap-2.5', 'text-emerald-800', 'font-bold', 'text-xs', 'uppercase', 'tracking-wider')}>
                                            ...
                                            <tbody className={clsx('bg-white', 'divide-y', 'divide-gray-100')}>
                                                {currentItems.map((seller) => {
                                                    const statusInfo = getStatusInfo(seller.status);
                                                    const StatusIcon = statusInfo.icon;
                                                    return (
                                                        <tr key={seller.seller_id || seller.id} className={clsx('hover:bg-emerald-50/50', 'transition', 'duration-150', 'group')}>
                                                            <td className={clsx('px-6', 'py-4', 'whitespace-nowrap')}>
                                                                <div className={clsx('font-medium', 'text-gray-900')}>
                                                                    {seller.business_name || "N/A"}
                                                                </div>
                                                                <div className={clsx('text-xs', 'text-gray-400')}>
                                                                    {seller.seller_id || "No ID"}
                                                                </div>
                                                            </td>
                                                            <td className={clsx('px-6', 'py-4', 'whitespace-nowrap')}>
                                                                <div className={clsx('text-sm', 'text-gray-700')}>
                                                                    {seller.owner_name || "N/A"}
                                                                </div>
                                                            </td>
                                                            <td className={clsx('px-6', 'py-4', 'whitespace-nowrap', 'hidden', 'md:table-cell')}>
                                                                <div className={clsx('text-sm', 'text-gray-600', 'flex', 'items-center', 'gap-1')}>
                                                                    <IoMailOutline size={14} className="text-gray-400" />
                                                                    {seller.email || "No email"}
                                                                </div>
                                                                <div className={clsx('text-sm', 'text-gray-500', 'flex', 'items-center', 'gap-1')}>
                                                                    <IoCallOutline size={14} className="text-gray-400" />
                                                                    {seller.mobile_no || "No phone"}
                                                                </div>
                                                            </td>
                                                            <td className={clsx('px-6', 'py-4', 'whitespace-nowrap')}>
                                                                <span className={clsx(
                                                            ...
                                                            <td className={clsx('px-6', 'py-4', 'whitespace-nowrap', 'text-center')}>
                                                                {isSuperAdmin ? (
                                                                    <div className={clsx('flex', 'items-center', 'justify-center', 'gap-2', 'flex-wrap')}>
                                                                        {updatingId === seller.seller_id ? (
                                                                            <div className={clsx('animate-spin', 'rounded-full', 'h-5', 'w-5', 'border-b-2', 'border-emerald-600')}></div>
                                                                        ) : (
                                                                            <>
                                                                                <button
                                                                    ...
                                                                    <span className={clsx('text-xs', 'text-gray-400', 'bg-gray-100', 'px-3', 'py-1', 'rounded-full')}>
                                                                        Read Only
                                                                    </span>
                                                                )}
                                                            </td>
                                    ...
                                    {/* Pagination */}
                                    {totalPages > 1 && (
                                        <div className={clsx('px-6', 'py-4', 'bg-gray-50', 'border-t', 'border-gray-200', 'flex', 'items-center', 'justify-between', 'flex-wrap', 'gap-4', 'mt-4', 'rounded-b-2xl')}>
            ...
            {showConfirmModal && confirmAction && (
                <div className={clsx('fixed', 'inset-0', 'z-50', 'flex', 'items-center', 'justify-center', 'bg-black/50', 'backdrop-blur-sm')}>
                    <div className={clsx('bg-white', 'rounded-2xl', 'shadow-2xl', 'max-w-md', 'w-full', 'mx-4', 'p-8', 'transform', 'transition-all', 'scale-100')}>
                        <div className="text-center">
                            <div className={clsx(
                                ...
                                confirmAction.newStatus === 4 ? "bg-indigo-100" :
                                "bg-gray-100"
                            )}>
                                ...
                                {confirmAction.newStatus === 4 && <IoRepeatOutline size={32} className="text-indigo-600" />}
                            ...
                            <p className={clsx('text-gray-600', 'mb-4')}>
                                ...
                                <span className={clsx("font-bold", confirmAction.statusColor)}>
                                    "{confirmAction.statusName}"
                                </span>
                                ?
                            </p>
                            <div className={clsx('flex', 'gap-3', 'justify-center')}>
                                <button
                                    onClick={cancelConfirm}
                                    className={clsx('px-6', 'py-2.5', 'bg-gray-100', 'hover:bg-gray-200', 'text-gray-700', 'rounded-xl', 'font-medium', 'transition')}
                                >
                                    Cancel
                                </button>
                                <button
                                    ...
                                    className={clsx(
                                        ...
                                        confirmAction.newStatus === 4 ? "bg-indigo-600 hover:bg-indigo-700" :
