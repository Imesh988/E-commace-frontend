...
import { Package, Truck, User, Search, Calendar, DollarSign, Hash, ShoppingBag, Eye, X, MapPin, Phone } from 'lucide-react';
...
const AdminOrdersPage = () => {
  ...
  const openModal = (order) => {
    console.log('Opening modal for order:', order.order_id, 'Items:', order.items);
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#f8faf9] via-white to-[#eef2f0]">
      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-5">
          <div className="">
            <div className="absolute -bottom-2 left-0 w-20 h-1 bg-emerald-400 rounded-full"></div>
          </div>
          <div className="relative w-full md:w-96">
            <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />
            <input type="text" placeholder="Search by name, email or order ID..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-12 pr-5 py-3 rounded-2xl border border-gray-200 bg-white/80 backdrop-blur-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition-all text-sm" />
          </div>
        </div>
        ...
        {filteredOrders.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-7">
            {filteredOrders.map(order => (
              <div key={order.order_id} className="group bg-white rounded-3xl border border-gray-100 shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden hover:-translate-y-1">
                <div className="relative p-5 pb-3 border-b border-gray-100 bg-gradient-to-r from-white to-gray-50/30">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                      ...
                      <div><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Order ID</p><p className="font-mono font-bold text-gray-800 text-sm">#{order.order_id.slice(-8).toUpperCase()}</p></div>
                    </div>
      ...
      {/* Modal - Fixed to show order items with product name */}
                ...
                <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
                  {selectedOrder.items && selectedOrder.items.length > 0 ? (
                    selectedOrder.items.map((item, idx) => {
                      ...
                      return (
                        <div key={idx} className="flex justify-between items-center bg-gray-50/80 p-3 rounded-xl hover:bg-gray-100 transition">
