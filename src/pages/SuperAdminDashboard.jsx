...
import { sellerApi, superAdminApi, userApi, roleApi } from '../services/api';
...
const SuperAdminDashboard = () => {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                ...
                const userRes = await userApi.getAllUser();
                const allUsers = userRes.data?.data || userRes.data || [];
                setUserCount(allUsers.length);

                const roleRes = await roleApi.getAllRole(); 
    ...
    const maxCount = Math.max(sellerCount, superAdminCount, userCount, roleCount, 10);
    const sellerLineY = 100 - (sellerCount / (maxCount * 1.2)) * 100;
    const superAdminLineY = 100 - (superAdminCount / (maxCount * 1.2)) * 100;
    const userLineY = 100 - (userCount / (maxCount * 1.2)) * 100; 
                ...
                <section>
                    ...
                    <div className="flex flex-wrap gap-4 mb-10">
                        <button
                            ...
                            className="flex items-center gap-3 bg-green-400 text-white px-6 py-3.5 rounded-2xl shadow-lg shadow-green-100 hover:bg-green-500 transition-all hover:-translate-y-1">
                        ...
                        <button
                            ...
                            className="flex items-center gap-3 bg-yellow-200 text-yellow-900 px-6 py-3.5 rounded-2xl shadow-lg shadow-yellow-100 hover:bg-yellow-300 transition-all hover:-translate-y-1">
                            <div className="bg-white/20 p-1 rounded-md"><Plus size={18} strokeWidth={3} /></div>
                            <span className="font-bold">Create New Super Admin</span>
                        </button>
                        <button
                            onClick={() => navigate('/user')} 
                            className="flex items-center gap-3 bg-[#5bd4ca] text-white px-6 py-3.5 rounded-2xl shadow-lg shadow-[#5bd4ca]/30 hover:bg-[#34b1a6] transition-all hover:-translate-y-1">
                            <div className="bg-white/20 p-1 rounded-md"><IoIosEye size={18} /></div> 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        <div className="bg-white p-8 rounded-[35px] border border-gray-100 shadow-sm relative overflow-hidden group transition-all hover:shadow-xl">
                            ...
                            <div className="bg-emerald-50 w-12 h-12 rounded-2xl flex items-center justify-center mb-6">
                                <RiUserShared2Line size={24} className="text-emerald-600" />
                            </div>
                            <p className="text-xs font-bold text-gray-400 tracking-wider">TOTAL SELLERS OR SUPPLIER</p>
                        ...
                        <div className="bg-[#fffbeb] p-8 rounded-[35px] border border-yellow-100 shadow-sm relative overflow-hidden group transition-all hover:shadow-xl">
                            ...
                            <div className="bg-white w-12 h-12 rounded-2xl flex items-center justify-center mb-6">
                                <FaUserShield size={24} className="text-yellow-600" />
                            </div>
                        ...
                        <div className="bg-blue-50 p-8 rounded-[35px] border border-blue-100 shadow-sm relative overflow-hidden group transition-all hover:shadow-xl">
                            <div className="absolute top-0 right-0 p-6">
                                <BarChart3 size={28} className="text-blue-500 opacity-20 group-hover:opacity-100 transition-opacity" />
                            </div>
                            <div className="bg-white w-12 h-12 rounded-2xl flex items-center justify-center mb-6">
                                <MdOutlinePeopleAlt size={24} className="text-blue-600" />
                            </div>
                            <p className="text-xs font-bold text-gray-400 tracking-wider">TOTAL USERS</p>
                            <p className="text-3xl font-black text-gray-800 mt-1">
                                {userCount.toLocaleString()}
                            </p>
                            <div className="absolute -bottom-6 -right-6 w-32 h-32 bg-blue-100 rounded-full blur-3xl opacity-30"></div>
                        </div>

                        <div className="bg-purple-50 p-8 rounded-[35px] border border-purple-100 shadow-sm relative overflow-hidden group transition-all hover:shadow-xl">
                            <div className="absolute top-0 right-0 p-6">
                                <BarChart3 size={28} className="text-purple-500 opacity-20 group-hover:opacity-100 transition-opacity" />
                            </div>
                            <div className="bg-white w-12 h-12 rounded-2xl flex items-center justify-center mb-6">
                                <GrUserAdmin size={24} className="text-purple-600" />
                            </div>
                            <p className="text-xs font-bold text-gray-400 tracking-wider">TOTAL ROLES</p>
                            <p className="text-3xl font-black text-gray-800 mt-1">
                                {roleCount.toLocaleString()}
                            </p>
                            <div className="absolute -bottom-6 -right-6 w-32 h-32 bg-purple-100 rounded-full blur-3xl opacity-30"></div>
                        </div>
                    </div>
                </section>
                ...
                <section className="bg-white p-10 rounded-[45px] border border-gray-100 shadow-sm transition-all hover:shadow-md">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                        <div>
                            ...
                            <p className="text-xs font-bold text-gray-400 tracking-widest">SELLERS VS SUPER ADMINS VS USERS ONBOARDING TREND</p> 
                        </div>
                        ...
                        <div className="flex flex-wrap gap-6 text-xs font-bold"> 
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-gray-500">Users ({userCount})</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="h-64 w-full relative mb-10">
                        <svg viewBox="0 0 1000 100" preserveAspectRatio="none" className="w-full h-full drop-shadow-sm">
                            ...
                            <path
                                d={`M0,95 Q250,${userLineY + 5} 500,${userLineY - 5} T1000,${userLineY - 15}`} 
                                fill="none"
                                stroke="#60a5fa" 
                                strokeWidth="4"
                                strokeLinecap="round"
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-8 border-t border-gray-100 pt-8"> 
                        <div className="group">
                            <p className="text-[10px] font-bold text-gray-400 tracking-widest uppercase mb-1">Total Seller Or supplier </p>
                        ...
                        <div className="group">
                            <p className="text-[10px] font-bold text-gray-400 tracking-widest uppercase mb-1">Total User Network</p>
                            <div className="flex items-baseline gap-2">
                                <p className="text-2xl font-black text-gray-800">{userCount}</p>
                                <span className="text-[10px] font-bold text-blue-500 bg-blue-50 px-2 py-0.5 rounded-full">★ Members</span>
                            </div>
                        </div>
                    </div>
                </section>
